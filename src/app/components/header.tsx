...
export function Header() {
  ...
  const navItems = isWatch ? WATCH_NAV : RADAR_NAV;
  ...
  const switchTarget = isWatch
    ? { href: "/", label: "Idea Radar" }
  ...
  return (
    <header className="bg-ink text-canvas sticky top-0 z-40">
      <div className="max-w-[1280px] mx-auto px-5 md:px-10 h-16 md:h-[72px] flex items-center justify-between gap-4">
        <Link href={isWatch ? "/watch" : "/"} className="flex items-baseline gap-3 shrink-0">
          ...
          <span className="hidden lg:block text-[10px] uppercase tracking-[0.2em] text-canvas/50 translate-y-[-2px]">
            {isWatch ? "Estonian AI Field" : "Personal Intelligence"}
          </span>
        </Link>
      ...
      {open && (
        ...
        >
          ...
          <Link
            href={switchTarget.href}
            className="flex items-center gap-2 px-5 py-3.5 text-[15px] font-semibold border-t border-canvas/15 text-canvas bg-canvas/5"
          >
            <span aria-hidden className="w-1.5 h-1.5 rounded-full bg-ochre" />
            {switchTarget.label}
          </Link>
